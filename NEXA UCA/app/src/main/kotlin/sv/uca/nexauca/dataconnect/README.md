...
for the Firebase Data Connect connector named `nexa`,
with service ID `nexauca`
...
import sv.uca.nexauca.dataconnect.*
```
...
## NexaConnector - The Main Class

The [NexaConnector] class is the main entry point
for accessing the `nexa` Data Connect connector.
...
Finally, it provides a [NexaConnector.dataConnect] property
that provides access to the underlying
[com.google.firebase.dataconnect.FirebaseDataConnect] instance.

### NexaConnector - Retrieving Instances

The default instance of [NexaConnector] can be retrieved,
creating it if it does not already exist, by accessing the [instance] extension
property of the companion object of [NexaConnector].
For example,

```kotlin
val connector: NexaConnector = NexaConnector.instance
```

If the default [NexaConnector] instance is not sufficient,
...
```kotlin
val connector: NexaConnector = NexaConnector.getInstance(
...
is the query named "GetActividadReciente",
which can be accessed via the [NexaConnector.getActividadReciente] property.
...
The [NexaConnector.dataConnect] property
...
val connector1 = NexaConnector.instance
val connector2 = NexaConnector.instance
check(connector1 === connector2)

connector1.dataConnect.close()

// After close(), the `instance` property returns a new instance.
val connector3 = NexaConnector.instance
...
of the corresponding property of [NexaConnector].
...
For example, the "GetCareers" query has no variables
and can be executed via the
[NexaConnector.getCareers]
property as follows:

```kotlin
val connector = NexaConnector.instance
val queryResult = connector.getCareers.execute()
println("GetCareers query returned: ${queryResult.data}")
```
...
For example, the "GetAsistenciasSemana" query has 3 required variables ("uid", "inicioSemana", and "finSemana")
and can be executed via the [NexaConnector.getAsistenciasSemana]
property as follows:

```kotlin
val connector = NexaConnector.instance
val queryResult = connector.getAsistenciasSemana.execute(uid="qux", inicioSemana=Timestamp(2102575245, 797174942), finSemana=Timestamp(1490695928, 309075241))
println("GetAsistenciasSemana query returned: ${queryResult.data}")
```
...
For example, the "CreateStudent" mutation has 2 optional variables ("birthDate" and "careerId")
and can be executed via the [NexaConnector.createStudent]
property as follows:

```kotlin
val connector = NexaConnector.instance
val mutationResult = connector.createStudent.execute(studentCode="qux", phoneNumber="thud", userId="qux") {
  birthDate = LocalDate(1765, 7, 2)
  careerId = java.util.UUID.randomUUID()
}
println("CreateStudent mutation returned: ${mutationResult.data}")
```
