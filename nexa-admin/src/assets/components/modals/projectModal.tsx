...
import { GeneralAlert } from '../Alerts/Alerts';
...
export default function ModalProject({ isOpen, onClose }: ModalProps) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const allProjects = await ProgramService.getAllProjects();
            ...
            if (exists) {
                setAlertConfig({ type: 'error', title: 'Error', message: 'Ya existe un programa con este nombre' });
                setAlertOpen(true);
                return;
            }
            ...
            await ProgramService.createProgram(projectData);


            setAlertConfig({ type: 'success', title: 'Éxito', message: 'Programa registrado correctamente.' });
            setAlertOpen(true);
        } catch (error) {
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                    ...
                    >
                        ...
                        <DialogActions sx={{
                            ...
                            <button type='submit' onClick={handleSubmit} className='relative group overflow-hidden rounded-lg border border-zinc-300 px-6 py-2 text-black transition-all duration-300 cursor-pointer'>
