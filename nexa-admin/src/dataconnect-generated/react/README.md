...
This README will guide you through the process of using the generated React SDK package for the connector `nexa`. It will also provide examples on how to use your generated SDK to call your Data Connect queries and mutations.
...
This SDK provides [React](https://react.dev/) hooks generated specific to your application, for the operations found in the connector `nexa`. These hooks are generated using [TanStack Query Firebase](https://react-query-firebase.invertase.dev/) by our partners at Invertase, a library built on top of [TanStack React Query v5](https://tanstack.com/query/v5/docs/framework/react/overview).
...
A connector is a collection of Queries and Mutations. One SDK is generated for each connector - this SDK is generated for the connector `nexa`.
...
Below are examples of how to use the `nexa` connector's generated Query hook functions to execute each Query. You can also follow the examples from the [Data Connect documentation](https://firebase.google.com/docs/data-connect/web-sdk#operations-react-angular).
...
## GetCareers
You can execute the `GetCareers` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetCareers(dc: DataConnect, options?: useDataConnectQueryOptions<GetCareersData>): UseDataConnectQueryResult<GetCareersData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetCareers(options?: useDataConnectQueryOptions<GetCareersData>): UseDataConnectQueryResult<GetCareersData, undefined>;
```

### Variables
The `GetCareers` Query has no variables.
### Return Type
Recall that calling the `GetCareers` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetCareers` Query is of type `GetCareersData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetCareers`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@dataconnect/generated';
import { useGetCareers } from '@dataconnect/generated/react'

export default function GetCareersComponent() {
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetCareers();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetCareers(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetCareers(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetCareers(dataConnect, options);
...
```javascript
useGetDepartment(dc: DataConnect, options?: useDataConnectQueryOptions<GetDepartmentData>): UseDataConnectQueryResult<GetDepartmentData, undefined>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetDepartment(options?: useDataConnectQueryOptions<GetDepartmentData>): UseDataConnectQueryResult<GetDepartmentData, undefined>;
```
...
export default function GetDepartmentComponent() {
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetDepartment();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetDepartment(dataConnect);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetDepartment(options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetDepartment(dataConnect, options);
...
## GetRoleByName
You can execute the `GetRoleByName` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetRoleByName(dc: DataConnect, vars: GetRoleByNameVariables, options?: useDataConnectQueryOptions<GetRoleByNameData>): UseDataConnectQueryResult<GetRoleByNameData, GetRoleByNameVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetRoleByName(vars: GetRoleByNameVariables, options?: useDataConnectQueryOptions<GetRoleByNameData>): UseDataConnectQueryResult<GetRoleByNameData, GetRoleByNameVariables>;
```
...
```
### Return Type
Recall that calling the `GetRoleByName` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetRoleByName` Query is of type `GetRoleByNameData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetRoleByName`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetRoleByNameVariables } from '@dataconnect/generated';
import { useGetRoleByName } from '@dataconnect/generated/react'

export default function GetRoleByNameComponent() {
  // The `useGetRoleByName` Query hook requires an argument of type `GetRoleByNameVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetRoleByName(getRoleByNameVars);
  // Variables can be defined inline as well.
  const query = useGetRoleByName({ name: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetRoleByName(dataConnect, getRoleByNameVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetRoleByName(getRoleByNameVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetRoleByName(dataConnect, getRoleByNameVars, options);
...
Below are examples of how to use the `nexa` connector's generated Mutation hook functions to execute each Mutation. You can also follow the examples from the [Data Connect documentation](https://firebase.google.com/docs/data-connect/web-sdk#operations-react-angular).
...
export default function CreateUserComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ id: ..., email: ..., fullName: ..., photoUrl: ..., roleId: ..., });
...
export default function CreateStudentComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ studentCode: ..., phoneNumber: ..., birthDate: ..., userId: ..., careerId: ..., });
